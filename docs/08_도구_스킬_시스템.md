...
### ToolExecutor — 공통 실행 게이트

`ToolExecutor`는 모든 도구 실행 전에 다음 검증을 수행한다.

- 도구 등록 여부 및 활성화 상태 확인
- `ToolParameterValidator` 기반 JSON Schema 검증: 필수값, 타입, enum, 문자열 길이, 숫자 범위
- `Tool.getRequiredPermissions()`에 선언된 Android runtime permission 허용 여부 확인
- 채팅 경로에서 전달된 사용자 메시지 ID가 있으면 `tool_calls` 테이블에 도구명, 파라미터, 결과, 성공/실패, 실행 시간을 저장
- 검증 실패 시 도구 구현체를 호출하지 않고 실패 `ToolResult` 반환
